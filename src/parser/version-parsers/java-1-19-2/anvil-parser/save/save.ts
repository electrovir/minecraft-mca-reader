 ...
 */
async function writeDirectoryToZip(
    d: DirectoryEntry,
    z: JSZip,
    path: string,
    overrideMap?: Map<string, ArrayBuffer>,
): Promise<void> {
    return new Promise((resolve, reject) => {
        d.createReader().readEntries(
            (entries) =>
                Promise.all<void>(
                    entries.map((e) => {
                        const thispath = `${path}/${e.name}`;
                        if (overrideMap?.has(thispath))
                            // if this path has been manually overridden with data, write that
...
/**
 * Provides methods for navigating a Minecraft world save directory. Corresponding region files and
 * the level NBT tag can be read, mutated, and saved.
 */
export class SaveParser {
    ...
    /**
     * Returns an region file reference for the region within this world containing the given
     * coordinate. If the region does not exist (i.e. it has not yet been rendered), the function
     * returns undefined.
     *
     * @param coordinate The coordinates for which to retrive the parser.
     * @returns A region file reference with the region data if the region exists; undefined
     *   otherwise.
     */
    async getRegionFileContainingCoordinate(
        coordinate: Coordinate3D,
    ): Promise<RegionFile | undefined> {
        if (this.cachedRegions.size === 0)
            this.cachedRegions = associateBy(
    ...
    /**
     * Returns an Anvil parser for reading and mutating the region within this world containing the
     * given coordinate. If the region does not exist (i.e. it has not yet been rendered), the
    ...
    /**
     * Places a new block at the specified coordinates within this world. If the region containing
     * the coordinate does not exist within this world (i.e. it has not yet been rendered), no
    ...
    async setBlock(coordinates: Coordinate3D, name: string, properties: {[key: string]: string}) {
        (await this.getAnvilParserByCoordinate(coordinates))?.setBlock(
            coordinates,
            name,
            properties,
        );
    ...
    /**
     * Fetches a block from the specified coordinates within this world. If the region containing
     * the coordinate does not exist within this world (i.e. it has not yet been rendered),
     * undefined will be returned.
     *
     * @param coordinates The coordinates from which to fetch the block.
     * @returns Object containing the name and key-value properties of the block if the region is
    ...
    /**
     * Writes this file, with any updates to regions and chunks reflected, to an in-memory ZIP file
     * for download.
     */
    async asZip(): Promise<JSZip> {
        const dirtyRegions = [...this.dirtyRegions.keys()];
        const zip = new (JSZip as any)() as JSZip;
        const overrideMap = new Map(
            dirtyRegions.map((key) => [
