...
import {paletteAsList, paletteBlockList, paletteNameList} from '../block';
...
export function sectionBlockStates(section: TagData[]): BlockStates | undefined {
    const b =
        section.find((x) => x.name === 'BlockStates') ||
        (section.find((x) => x.name === 'block_states')?.data as TagData[]).find(
            (x) => x.name === 'data',
        );
...
export function sectionPalette(section: TagData[]): Palette | undefined {
    const p =
        section.find((x) => x.name === 'Palette' || x.name === 'palette') ||
        (section.find((x) => x.name === 'block_states')?.data as TagData[]).find(
            (x) => x.name === 'palette',
        );
...
/**
 * Class for parsing, mutating, and writing chunk-format data to and from NBT-format blobs. This
 * class handles NBT-related parsing and writing only; chunk compression logic is handled by the
 * AnvilParser class.
 *
...
export class Chunk {
    ...
    /**
     * Checks if the given NBT tag is a valid chunk section tag (containing a list of chunk
     * sections).
     *
    ...
    static isValidChunkSectionTag(tag?: TagData): tag is ChunkSectionTag {
        ...
        return (
            t &&
            t.type === TagType.LIST &&
            t.name.toLowerCase() === 'sections' &&
            t.data &&
            t.data.subType === TagType.COMPOUND &&
            t.data.data &&
            t.data.data.length !== undefined
        );
    }
    ...
    static isValidChunkRootTag(tag?: TagData): tag is ChunkRootTag {
        ...
        return (
            ...
            t.name === '' &&
            t.data &&
            t.data.length !== undefined
        );
    }

    /**
     * Creates a zero-filled 2D array of integers for storing blocks in an x-coordinate column of a
     * chunk or section.
     *
     * @param y The total number of y-coordinates to represent.
     * @param z The total number of z-coordinates to represent.
    ...
    getChunkCoordinates(): [number, number] | undefined {
        ...
        const x = (
            findChildTagAtPath('Level/xPos', this.root) || findChildTagAtPath('xPos', this.root)
        )?.data;
        const z = (
            findChildTagAtPath('Level/zPos', this.root) || findChildTagAtPath('zPos', this.root)
        )?.data;
    ...
    sections(): ChunkSectionTag | undefined {
        if (!Chunk.isValidChunkRootTag(this.root)) return;
        const sectionTag =
            findChildTagAtPath('Level/Sections', this.root) ||
            findChildTagAtPath('sections', this.root);
    ...
    sortedSections(): TagData[][] | undefined {
        ...
        const yTags = findCompoundListChildren(s!, (x) => x.name === 'Y')
            ?.map((v, i) => ({v, i}))
            .filter((x) => x.v);
        return yTags
            ?.sort((a, b) => (a.v!.data as number) - (b.v!.data as number))
            .map((x) => s!.data.data[x.i]!);
    }
    ...
    getBlock(coordinates: Coordinate3D): {name: string; properties: {[key: string]: string}} {
        const yIndex =
            coordinates[1] >= 0
                ? Math.floor(coordinates[1] / 16)
                : Math.floor((coordinates[1] + 4096) / 16);
        const [
            s,
            palette,
        ] = this.sectionBlockStateTensor(yIndex);
    ...
    setBlock(coordinates: Coordinate3D, name: string, properties: {[key: string]: string}) {
        const yIndex =
            coordinates[1] >= 0
                ? Math.floor(coordinates[1] / 16)
                : Math.floor((coordinates[1] + 4096) / 16);
        const fullName = `${name}(${Object.keys(properties)
            ...
            this.palettes.set(
                ...
                nbtTagReducer(
                    palette || {
                        type: TagType.LIST,
                        name: 'palette',
                        data: {subType: TagType.COMPOUND, data: []},
                    },
                    {
     ...
     * @returns A matrix of world heights; x-coordinate is the outer coordinate and z-coordinate is
    ...
    worldHeights(name: string = 'WORLD_SURFACE'): number[][] | undefined {
        ...
        const map =
            findChildTagAtPath(`Level/Heightmaps/${name}`, this.root) ||
            findChildTagAtPath(`Heightmaps/${name}`, this.root);
    ...
    /**
     * Extracts a 3D tensor containing block states for the chunk section at the given y coordinates
     * and the palette NBT tag.
     *
     * @param yIndex The y index of the coordinate (in chunk coordinates).
     * @returns A 3D tensor of block state indexes (corresponding to indexes in the block state
     *   palette for the section) and the palette NBT tag.
     */
    sectionBlockStateTensor(yIndex: number): [number[][][], Palette | undefined] {
        ...
        if (this.blockStates.get(yIndex) && this.palettes.get(yIndex))
            return [
                this.blockStates.get(yIndex)!,
                this.palettes.get(yIndex)!,
            ];
        const r: number[][][] = [];
        ...
        const section = sections.find(
            (x) => x.find((xx) => xx.name === 'Y')?.data === yIndex,
        ) as TagData[];
     ...
     *
     * @returns A 3D tensor of block states for the chunk, each corresponding to an index in the
     *   corresponding chunk section palette.
     */
    ...
    /**
     * Flushes unsaved changes to this chunk's NBT tag and then returns the tag. This method should
     * be used instead of directly accessing the object's root tag property to ensure any unsaved
     * block changes are reflected.
     *
    ...
    chunkData(): TagData {
        ...
        [...this.blockStatesDirty.keys()]
            .filter((k) => this.blockStatesDirty.get(k))
            .forEach((yy) => {
                ...
                const [
                    r,
                    palette,
                ] = BlockDataParser.writeBlockStates(blocks, this.palettes.get(yy)!);
                const index = sections.findIndex(
                    (x) => x.find((xx) => xx.name === 'Y')?.data === yy,
                );
                this.root =
                    (palette?.data.data.length || 0) > 1
                        ...
                        : findChildTagAtPath(`sections/${index}/block_states/data`, this.root)
                        ? nbtTagReducer(this.root, {
                ...
                this.root = nbtTagReducer(this.root, {
                    ...
                    overwrite: true,
                    path: `sections/${index}/block_states`,
                    tag: (this.palettes.get(yy) ||
                        findChildTagAtPath(`sections/${index}/block_states/palette`, this.root))!,
                }); // updates palette
