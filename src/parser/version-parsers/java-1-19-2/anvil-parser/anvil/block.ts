import {array as md5} from 'js-md5';
...
export function block(t: TagData[]): {name: string; properties: {[key: string]: string}} {
    ...
    ((t.find((x) => x.name.toLowerCase() === 'properties')?.data || []) as TagData[])
        .filter((x) => x.type === TagType.STRING)
        .forEach((x) => {
            properties[x.name] = x.data as string;
        });
    return {name, properties};
}

export function blockTypeID(t: TagData[]): number {
    const w = md5(blockTypeString(t));
    return w[0]!;
}

export function paletteNameList(palette: Palette): string[] {
    return palette.data.data.map(
        (x) => (x.find((x) => x.name.toLowerCase() === 'name')?.data || '') as string,
    );
}

export function paletteAsList(
    palette: Palette,
...
/**
 * Provides bitwise parsing and writing for the UInt64 array encoding block states within chunk
 * sections.
 */
export class BlockDataParser extends BitParser {
     ...
     *
     * @param states The list of block states to write, corresponding to indexes in the
     *   corresponding palette.
     * @returns An ArrayBuffer containing the data which can be inserted into a long array NBT tag.
     */
    static writeBlockStates(
        states: number[],
        palette?: Palette,
    ): [ArrayBuffer, Palette | undefined] {
        /* get list of blocks which appear at least once and reassign IDs if necessary */
        const blocks = states.map((state) =>
            palette ? blockTypeString(palette.data.data[state]!) : '' + state,
        );
        const new_id_map = new Map<string, number>(
            [...new Set(blocks)].map((x, i) => [
                x,
                i,
            ]),
        );
        const palette_map = new Map<string, TagData[]>(
            palette?.data.data.map((x) => [
                blockTypeString(x),
                x,
            ]),
        );
        const new_palette = palette
            ? [...new Set(blocks)]
                  .sort((a, b) => new_id_map.get(a)! - new_id_map.get(b)!)
                  .map((block) => palette_map.get(block)!)
            : undefined;
        palette =
            palette && new_palette
                ? {
        ...
        const paletteSize =
            palette?.data.data.length === undefined
                ? Math.max(...states)
                : palette!.data.data.length - 1;
     ...
     *
     * @param blockStates An NBT tag containing the block state data; typically found at
     *   block_states/data for a section.
     * @param palette An NBT tag containing the palette; typically found at block_states/palette for
     *   a section.
     */
    ...
    /**
     * Fetches block indexes stored in original format (prior to snapshot 20w17a) in which
     * individual blocks may be spread across multiple unsigned longs. Each index corresponds to an
     * index in the section's palette.
     *
    ...
    /**
     * Fetches block indexes stored in current format (after snapshot 20w17a) in which individual
     * blocks are not spread across multiple unsigned longs. Each index corresponds to an index in
     * the section's palette.
     *
     * @param f Transform function to apply to each fetched index.
     * @param limit Maximum number of blocks to retrieve; if not passed, all will be retrieved.
     * @returns List of transformed block indexes.
    ...
    private getBlockTypeIDMap() {
        this.blockTypeIDMap = associateBy(
    ...
    private getBlockTypeStringMap() {
        this.blockTypeStringMap = associateBy(
            this.palette.data.data,
            (x) => (x.find((x) => x.name.toLowerCase() === 'name')?.data || '') as string,
            blockTypeString,
        );
    ...
    /**
     * Fetches block indexes from the section, with each index corresponding to an index in the
     * section's palette. The original parameter may be used to specify whether the section was
     * generated prior to snapshot 20w17a, in which case the bitwise format encoding the blocks
     * differs.
     *
    ...
    /**
     * Fetches block NBT tags from the palette corresponding to the blocks in this section. The
     * original parameter may be used to specify whether the section was generated prior to snapshot
     * 20w17a, in which case the bitwise format encoding the blocks differs.
     *
     * @param original If set, specifies that the section was generated prior to snapshot 20w17a.
     * @param limit Maximum number of blocks to retrieve; if not passed, all will be retrieved.
    ...
    /**
     * Fetches block names from the palette corresponding to the blocks in this section. The
     * original parameter may be used to specify whether the section was generated prior to snapshot
     * 20w17a, in which case the bitwise format encoding the blocks differs.
     *
     * @param original If set, specifies that the section was generated prior to snapshot 20w17a.
    ...
    getBlockTypeNames(original?: boolean, limit?: number) {
        ...
        return original
            ? this.getBlocksGenericOriginal(
                  (x) => this.blockTypeStringMap?.get(this.paletteNames[x]!),
                  limit,
              )
            : this.getBlocksGeneric(
                  (x) => this.blockTypeStringMap?.get(this.paletteNames[x]!),
                  limit,
              );
    }
    ...
    getBlockTypeIDs(original?: boolean, limit?: number) {
        ...
        return original
            ? this.getBlocksGenericOriginal(
                  (x) => this.blockTypeIDMap?.get(this.paletteNames[x]!),
                  limit,
              )
    ...
    findBlocksByName(name: string): number[] {
        ...
        const nameIndex = findCompoundListChildren(
            this.palette,
            (x) => x.name.toLowerCase() === 'name',
        )
            ?.map((x, i) => ({x, i}))
            .find((x) => x.x?.data === name)?.i;
        return blocks
            .map((x, i) => ({x, i}))
            .filter((x) => x.x === nameIndex)
            .map((x) => x.i);
    }
}
